...
class Typewriter extends Element {
  constructor(metadata={props: {className: "dc-typewriter"}}) {
    super('div', metadata);
...
class Optdown extends Element {
  constructor(metadata = {}) {
    ...
    console.log("List", this.metadata.list);
    this.list = this.metadata.list || [];
    
    /* title - disabled non selectable to force selection */
    if (this.name.length > 0) {
      this.addChild(
        new Element("option", {
          props: { textContent: this.name, disabled: "", selected: "" }
        })
      );
    }
    
    // we want selected only
    for (const item of this.metadata.list) {
      this.addChild(
        new Element("option", {
          props: { value: item, textContent: capitalizeFirst(item) }
        })
      );
    }
  }
  
  onChange(e) { } // override for change events
...
if(registration) Optdown.register();