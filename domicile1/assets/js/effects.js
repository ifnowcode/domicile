const fxRegistry = {};
// start stop works on repeat
// BUG: clear works but re-start after clear doesn't work well or not at all. All classes will need updates to supports this. Been trying with Motes but this is a WIP.
class EffectOverlay extends Element {
  constructor(metadata = {}) {
    const merged = {
      ...
      css: {
        ...
        overflow: "hidden",
        zIndex: metadata?.css?.zIndex ?? "-1",   // was "999999" always on top
        ...(metadata.css || {}),
      },
    ...
    if (tracefx) console.log("Auto start:", this.autoStart);
    this.running = false;
  ...
  static register() {
    ...
    fxRegistry[this.name] = this;
    return this;
  }

  start() {
    if (tracefx) console.log("[effect] Start:", this.constructor.name, this.mounted, this.running);
  ...
  stop() {
    this.running = false;
    if (tracefx) console.log("[effect] Stop", this.running);
  ...
  // subclass overrides for visual cleanup
  clear() {
    if (tracefx) console.log("[effect] Clear", this.running);
  }

  loop() {
    if (this.running && this.mounted) {
      if (tracefx) console.log("[effect] loop: updating ...", this.running, this.mounted);
      this.update();
      //if (tracefx) console.log("[effect] Request animation frame");
  ...
  onMount() {
    if (this.autoStart) setTimeout(this.start.bind(this), 1);
  }
}

// was FloatingStars but stars don't float
// WIP - updating to clear and restart
class Motes extends EffectOverlay { 
  constructor(metadata = {}) {
    super(metadata);
    this.motes = [];
    this.count = metadata.count || 100;
  }
  
  // to support restart after clear
  start() { // was in constructor, added refresh and super.start
    if (this.motes.length === 0) {
      if (tracefx) console.log("Creating Motes");
      for (let i = 0; i < this.count; i++) {
        const mote = new Element("div", {
          css: {
            position: "absolute",
            width: "4px",
            height: "4px",
        ...
        this.motes.push({
          ...
          // create random numbers between -0.25 and +0.25
  ...
  update() {
    if (tracefx) console.log("Update Motes", this.running, this.mounted, this.motes);
    if (tracefx) console.log("Update Motes", window.innerWidth, window.innerHeight);
    ...
    for (const s of this.motes) {
      ...
      //if (tracefx) console.log(s.vx, s.vy);
      ...
      const opacity = (Math.sin(s.blinkPhase) + 1) / 2;
      s.el.dom.style.opacity = opacity;
    }
  }
  
  clear() {
    for (const s of this.motes) this.removeChild(s.el);
    this.motes = [];
    super.clear();
  }
}

if(registration) Motes.register();

class DebugMotes extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.motes = [];
    this.count = metadata.count || 100;
  ...
  update() {
    ...
    for (const m of this.motes) {
      ...
      // move dot
      m.dot.dom.style.transform = `translate(${m.x}px, ${m.y}px)`;

      // update label text + position
      m.label.dom.textContent = `${Math.round(m.x)},${Math.round(m.y)}`;
      m.label.dom.style.transform = `translate(${m.x + 6}px, ${m.y - 4}px)`;
      
      // blink - not working yet
      m.blinkPhase += m.blinkSpeed;
      const opacity = (Math.sin(m.blinkPhase) + 1) / 2;
      console.log("Opacity", m, opacity);
      m.dot.dom.style.opacity = opacity;
      m.label.dom.style.opacity = opacity;
    }
  }

  clear() {
    for (const m of this.motes) {
      this.removeChild(m.dot);
      this.removeChild(m.label);
    }
    this.motes = [];
    super.clear();
  }
...
if(registration) DebugMotes.register();

class FallingConfetti extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 80;
    this.pieces = [];
  ...
  clear() {
    for (const h of this.pieces) h.el.dom.remove();
    this.pieces = [];
    this.count = 0;
    super.clear();
  }
...
if(registration) FallingConfetti.register();

class Rain extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 120;
    this.drops = [];
  ...
  clear() {
    for (const h of this.drops) h.el.dom.remove();
    this.drops = [];
    this.count = 0;
    super.clear();
  }
...
if(registration) Rain.register();

class BlueNeonRain extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 150;
    this.drops = [];
...
if(registration) BlueNeonRain.register();

class MatrixCodeRain extends EffectOverlay {
  //TODO: merge count into metadata so it's just metadata
  constructor(metadata = {}) {
    super(metadata);

    this.columns = [];
    this.count = metadata.count ?? 60;
  ...
  randomChar() {
    ...
    const chars = "00000000000000001111111111111111  ";
  ...
  clear() {
    for (const h of this.columns) h.el.dom.remove();
    this.columns = [];
  }
}

if(registration) MatrixCodeRain.register();
...
class GlowClick extends EffectOverlay {
  ...
  clear() {
    for (const h of this.ripples) h.el.remove();
    this.ripples = [];
...
if(registration) GlowClick.register();
...
class ItsRainingMoney extends EffectOverlay {
  ...
  clear() {
    for (const h of this.items) h.el.remove();
    this.items = [];
    //this.columnCount = 0;
    super.clear();
  }
...
if(registration) ItsRainingMoney.register();

class PartyBalloons extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 40;
    this.balloons = [];
  }
...
if(registration) PartyBalloons.register();
...
if(registration) Streamers.register();
...
if(registration) Sparkles.register();
...
class PartyCelebration extends EffectOverlay {
  constructor(metadata = {}) {
    ...
    this.counts = {
      balloonCount: merged.balloonCount,
      streamerCount: merged.streamerCount,
      confettiCount: merged.confettiCount,
      sparkleCount: merged.sparkleCount
    };
    
    console.log("meta Counts:", this.counts);
  }
...
if(registration) PartyCelebration.register();
...
if(registration) Fireworks.register();
...
if(registration) NewYearsEve.register();
...
if(registration) Inferno.register();
...
if(registration) Bubbles.register();

class Fireflies extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 50;
    this.fireflies = [];
  }
...
if(registration) Fireflies.register();
...
class FloatingHearts extends EffectOverlay {
  ...
  update() {
    const w = window.innerWidth;
    const h = window.innerHeight;
    console.log(this.hearts.length, this.count);
...
if(registration) FloatingHearts.register();
...
if(registration) Petals.register();
...
if(registration) PetalBurst.register();
...
class MagicSparkles extends EffectOverlay {
  ...
  clear() {
    for (const p of this.sparkles) p.el.remove();
    this.sparkles = [];
    super.clear();
  }
}

if(registration) MagicSparkles.register();
...
if(registration) PixelSnow.register();
...
if(registration) DustMotes.register();
...
if(registration) RainDropsOnGlass.register();
...
if(registration) StarTwinkle.register();

class GreenFog extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 20;
    this.fog = [];
  }
...
if(registration) GreenFog.register();
...
if(registration) LightningStorm.register();

class Sunbeam extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 8;
    this.beams = [];
  }
...
if(registration) Sunbeam.register();
...
if(registration) MagicTrails.register();

class UnderwaterCaustics extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 12;
    this.caustics = [];
  }
...
if(registration) UnderwaterCaustics.register();

class Aurora extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 6;
    this.ribbons = [];
  }
...
if(registration) Aurora.register();

class WarpSpeed extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 200;
    this.stars = [];
  }
...
if(registration) WarpSpeed.register();
...
if(registration) LavaGlow.register();

class Rainbow extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    this.count = metadata.count || 5;
    this.arcs = [];
  }
...
if(registration) Rainbow.register();

class PrismaticSpray extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 5;
    const colors = metadata.colors || [];
    this.colors = Array.isArray(colors) && colors.length > 0
      ? colors
  ...
  buildGradient() {
    // Ensure the gradient loops smoothly by repeating the first color at the end
    const stops = [...this.colors, this.colors[0]];
...
if(registration) PrismaticSpray.register();

class PrismaticWaves extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 5;
    const colors = metadata.colors || [];
    // Default to rainbow if user does not specify colors
    this.colors = Array.isArray(colors) && colors.length > 0
      ? colors
      : ["red", "orange", "yellow", "green", "cyan", "blue", "violet"];
  ...
  buildGradient() {
    // Repeat first color at end for smooth conic wrap
    const stops = [...this.colors, this.colors[0]];
    return `conic-gradient(${stops.join(", ")})`;
  }
  ...
  spawnArc() {
    ...
    // Each arc gets its own orbital wave parameters
    this.arcs.push({
      el,
      ...
      // orbital speed
      angleSpeed: 0.002 + Math.random() * 0.01,
  ...
  update() {
    ...
    for (const a of this.arcs) {
      ...
      const y = a.cy + Math.sin(a.angle) * a.radius;
...
if(registration) PrismaticWaves.register();

class PrismaticNebula extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);
    const { count, colors } = metadata;
    this.count = metadata.count || 12;
    this.colors = metadata.colors || [];

    // Default rainbow if user does not specify colors
  ...
  spawnCloud() {
    ...
    this.clouds.push({
      ...
      swirlSpeed: 0.002 + Math.random() * 0.01,
      swirlRadius: 20 + Math.random() * 60,
  ...
  update() {
    ...
    while (this.clouds.length < this.count) this.spawnCloud();
    ...
    for (const c of this.clouds) {
      ...
      // Swirl motion
      c.swirlPhase += c.swirlSpeed;
      const swirlX = Math.cos(c.swirlPhase) * c.swirlRadius;
      const swirlY = Math.sin(c.swirlPhase) * c.swirlRadius;

      // Opacity pulse
      c.pulse += c.pulseSpeed;
      const opacity = 0.1 + (Math.sin(c.pulse) + 1) * 0.1; // 0.1 → 0.3

      c.el.style.opacity = opacity;

      // Apply transform
      c.el.style.transform =
        `translate(${c.x + swirlX}px, ${c.y + swirlY}px)`;
    }
  }
...
if(registration) PrismaticNebula.register();
...
if(registration) RainbowUnicornMagic.register();
...
if(registration) FloatingEmoji.register();
...
if(registration) FloatingNeonWord.register();
...
// just a fuzzier version of RainDropsOnGlass
class SoftGlowDrift extends EffectOverlay {
  constructor(metadata = {}) {
    super(metadata);

    this.count = metadata.count || 40;
    this.color = metadata.color || "rgba(255,255,255,0.8)";
    this.glows = [];
  }
  ...
  update() {
    ...
    while (this.glows.length < this.count) this.spawnGlow();
    ...
    for (const g of this.glows) {
      ...
      const opacity = 0.2 + (Math.sin(g.pulse) + 1) * 0.15;
  ...
  clear() {
    for (const g of this.glows) g.el.remove();
    this.glows = [];
    super.clear();
  }
}

if(registration) SoftGlowDrift.register();